...
char *usagestr = 
 ...
 " -R size	[128]  Maximum Read Ahead Buffers. Default = 128M.\n"
 " -W size	[128]  Maximum Write Buffers. Default = 128M.\n"
 " -C size	[128]  Maximum Chunk Buffers. Default = 128M.\n"
 " -M size		   Maximum Memory. \n"
...
ChunkerStartup(void)
{
	...
	while (chunkcount) {
		...
		if (debug)
		  ...
		  //With this simple rounding, mod 10 of progress will print the same number 4 times.  
		  //Print every 4th.
		  if (progress % 10 == 0 && progress == counter) {
