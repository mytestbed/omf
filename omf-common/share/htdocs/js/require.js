...
(function(_, undefined) { _ = _ || window;
	...
	function require(arr, onready) {
	  require2(arr, {}, onready);
	} _.require = require;

	function require2(arr, opts, onready) {
		if (typeof arr === 'string') { arr = [ arr ]; } // make sure we have an array
...
(function(_, undefined) { _ = _ || window;
  ...
  function require_obj(objName, opts, onready) {
    if (objName == undefined) { throw new Error("Missing 'objName'"); }

    var url = opts.url;
    if (url == undefined) { url = objName; }
    if (typeof url === 'string') { url = [ url ]; }
    url.reverse(); /* pop nibbles from teh back, but dependencies are from teh front. */
    var fa;
    if ((fa = obj2func[objName]) == undefined) {
      fa = obj2func[objName] = [];
    }
    fa.unshift(onready);
    _require(url);
  } _.require_obj = require_obj;
  ...
  function _require(url) {
    if (url.length > 0) {
      ...
      /* everything loaded, nw check if we loaded any new obj for which we have funcs. */
      _check_functions();
    }
  }

  function _check_functions() {
    for (var obj in obj2func) {
      if ((eval("typeof " + obj + ";")) != 'undefined') {
	var fa = obj2func[obj];
	...
	delete obj2func[obj];
	/* start over again as we may have loaded some new functions */
	_check_functions();
      }
    }
  }
