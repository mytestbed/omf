...
class AMTest::AM_API < Rack::RPC::Server
  ...
  def self.rpc2(rpc_name, method_name, opts = {}, &block)
    ...
    wrapper_name = "_wrapper_#{method_name}".to_sym
    self.send(:define_method, wrapper_name) do |*args|
      self.class.hooks[:before].each do |command| 
        command.call(self) if command.callable?(method_name)
      end
      out = self.send(method_name, *args)
      self.class.hooks[:after].each do |command| 
        command.call(self) if command.callable?(method_name)
      end
      out
    end
    @@mappings[rpc_name.to_s] = wrapper_name
  end
